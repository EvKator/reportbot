...
class Admin {
    ...
    static NewDocumentNotification(user) {
        return __awaiter(this, void 0, void 0, function* () {
            let admin = yield Admin.getAdmin();
            yield telegram_connection_1.bot.sendMessage(admin.id, `user ${user.id} created public template: `);
            yield telegram_connection_1.bot.sendDocument(user.id, user.templates[user.templates.length - 1].path);
        });
    }
    static IsAdmin(user) {
